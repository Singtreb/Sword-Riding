  ...
  return (
    <div className="space-y-6">
      <header className="flex flex-col gap-1">
        <h1 className="font-sans text-2xl font-semibold tracking-tight">运行实例</h1>
        <p className="text-sm text-muted-foreground">
          活动扫描实例和全局主机压力。已完成的扫描是历史记录。
        </p>
      </header>
      ...
      {error ? (
        <ErrorState
          ...
          action={<Button size="sm" variant="outline" onClick={() => refetch()}>重试</Button>}
        />
      ) : isLoading ? (
        ...
        <>
          ...
          {!showFilterBar ? (
            <EmptyState
              title="没有实例"
              description="开始扫描后，它会作为运行中的实例显示在这里。"
            />
          ) : (
            <>
              <Card>
